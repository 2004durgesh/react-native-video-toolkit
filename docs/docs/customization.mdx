...
| -------- | -------------------------------------------------------------------- | ------- | --------------------------------------------------- |
| `theme`  | [`Theme`](./api-reference/types/theme)                               | —       | Custom theme object for colors, fonts, sizing, etc. |
| `config` | [`VideoPlayerConfig`](./api-reference/types/video#videoplayerconfig) | —       | Configuration options for player behavior.          |
...
> ⚠️ When overriding gestures, ensure they don't conflict with system gestures (like swipe-back on iOS).
> 💡 Prefer using the provided **compound components** before re-implementing controls from scratch.
> 🎨 Themes should be consistent across the app — define once in `VideoProvider` for best maintainability.
