---
sidebar_label: 'Title'
sidebar_position: 1
---

# Title

`Title` is a **themed text component** for displaying headings, labels, or titles inside the video player UI.  
It automatically picks up colors from the player's active theme, ensuring visual consistency.

---

## Usage

```tsx
import { Title } from 'react-native-media-toolkit';

export function Example() {
  return <Title text="Playback Settings" />;
}
```

---

## Props

| Prop    | Type                   | Required | Default | Description                                              |
| ------- | ---------------------- | -------- | ------- | -------------------------------------------------------- |
| `text`  | `string`               | Yes      | –       | The string to display.                                   |
| `style` | `StyleProp<TextStyle>` | No       | –       | Optional override styles (merged with theme + defaults). |

---

## Theming

- Text color is derived from `theme.colors.text` in the global player state.
- Font size defaults to `16` and weight `bold`.
- Can be extended via `style` if you need variants (e.g., subtitles, secondary labels).

---

## Notes

:::info[When to Use]

- Use `Title` for **primary headings** or **main labels** within the video player UI, such as:
  - Video titles
    :::
