...
```tsx
import { VideoSurface } from 'react-native-media-toolkit';
...
| `...rest` | `ReactVideoProps`      | No       | –       | Any extra props are passed through to `react-native-video`. |
...
- **`onLoad`** → sets duration + clears buffering
